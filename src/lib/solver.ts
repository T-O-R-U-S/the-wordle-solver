...
export class Some {
	...
	apply(text: string) {
		if(text[this.pos] != this.char && text.includes(this.char)) {
			return true
		}
		return false
	}
...
export function dict(criteria: condition[], words: string[] = default_words): string[] {
	...
	words = words.sort(
		(a: any, b: any) => {
			...
			for (let chr of a) {
				if (!a_set.includes(chr)) {
					a_set.push(chr);
				}
			}

			for (let chr of b) {
                if (!b_set.includes(chr)) {
                    b_set.push(chr);
                }
			}

            a_weight += a_set.length * 3;
            b_weight += b_set.length * 3;

            // Accencuate diff
            a_weight *= 2;
            b_weight *= 2;

            for (let chr of a_set) {
                a_weight += letter_weight[chr] ?? -1;
            }

            for (let chr of b_set) {
                b_weight += letter_weight[chr] ?? -1;
            }
